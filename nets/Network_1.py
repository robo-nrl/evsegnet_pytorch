...
class Xception(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.block12=Block(728,728,2,1,start_with_relu=True,grow_first=False) ####
...
class Segception_small(nn.Module):
    def __init__(self, num_classes=6, input_shape=(3, None, None), weights='imagenet'):
        ...
        self.adap_encoder_2 = EncoderAdaption(in_channels=1024, out_channels=128, kernel_size=3, dilation=1)
        self.adap_encoder_3 = EncoderAdaption(in_channels=728, out_channels=128, kernel_size=3, dilation=1)
        self.adap_encoder_4 = EncoderAdaption(in_channels=256, out_channels=64, kernel_size=3, dilation=1)
        self.adap_encoder_5 = EncoderAdaption(in_channels=128, out_channels=32, kernel_size=3, dilation=1)
...
if __name__=="__main__":
    ...
    inp=torch.rand(8,6,352,224)
    # feats = xception_out(inputs=inp)
    # xception = Xception(num_classes=6)
    segception = Segception_small(num_classes=6, input_shape=(6,None, None))

    # inp = inp.to(device)
    # segception = segception.to(device)
    out = segception(inp)
    print(out.shape)
